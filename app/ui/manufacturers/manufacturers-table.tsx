'use client';

import { useState } from 'react';
import { Button } from '@/app/ui/button';
import { formatDateToLocal } from '@/app/lib/utils';
import { 
  PencilIcon, 
  TrashIcon, 
  EyeIcon, 
  EyeSlashIcon,
  PlusIcon 
} from '@heroicons/react/24/outline';
import type { Manufacturer } from '@/app/lib/definitions/manufacturer';

interface ManufacturersTableProps {
  manufacturers: Manufacturer[];
  onEdit: (manufacturer: Manufacturer) => void;
  onDelete: (manufacturerId: string) => void;
  onToggleActive: (manufacturerId: string) => void;
  onCreateNew: () => void;
}

export default function ManufacturersTable({
  manufacturers,
  onEdit,
  onDelete,
  onToggleActive,
  onCreateNew,
}: ManufacturersTableProps) {
  const [deletingId, setDeletingId] = useState<string | null>(null);

  const handleDelete = async (manufacturerId: string) => {
    if (window.confirm('Are you sure you want to delete this manufacturer?')) {
      setDeletingId(manufacturerId);
      await onDelete(manufacturerId);
      setDeletingId(null);
    }
  };

  return (
    <div className="mt-6 flow-root">
      <div className="inline-block min-w-full align-middle">
        <div className="rounded-lg bg-gray-50 p-2 md:pt-0">
          {/* Header with Create Button */}
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-900">Manufacturers</h2>
            <Button
              onClick={onCreateNew}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white"
            >
              <PlusIcon className="h-4 w-4" />
              Create Manufacturer
            </Button>
          </div>

          {/* Mobile View */}
          <div className="md:hidden">
            {manufacturers.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No manufacturers found
              </div>
            ) : (
              manufacturers.map((manufacturer) => (
                <div key={manufacturer.id} className="mb-2 w-full rounded-md bg-white p-4">
                  <div className="flex items-center justify-between border-b pb-4">
                    <div className="space-y-2">
                      <p className="text-sm font-medium text-gray-900">
                        {manufacturer.name}
                      </p>
                      <p className="text-xs text-gray-500">
                        {manufacturer.code} • {manufacturer.country}
                      </p>
                    </div>
                    <div className={`px-2 py-1 text-xs rounded-full ${
                      manufacturer.active 
                        ? 'bg-green-100 text-green-800' 
                        : 'bg-red-100 text-red-800'
                    }`}>
                      {manufacturer.active ? 'Active' : 'Inactive'}
                    </div>
                  </div>
                  <div className="flex w-full items-center justify-between pt-4">
                    <div className="space-y-2">
                      <p className="text-xs text-gray-500">
                        Created: {formatDateToLocal(manufacturer.created_at)}
                      </p>
                      {manufacturer.email && (
                        <p className="text-xs text-gray-500">
                          {manufacturer.email}
                        </p>
                      )}
                    </div>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => onEdit(manufacturer)}
                        className="text-blue-600 hover:text-blue-800"
                        title="Edit manufacturer"
                      >
                        <PencilIcon className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => onToggleActive(manufacturer.id)}
                        className={manufacturer.active 
                          ? 'text-orange-600 hover:text-orange-800' 
                          : 'text-green-600 hover:text-green-800'
                        }
                        title={manufacturer.active ? 'Deactivate' : 'Activate'}
                      >
                        {manufacturer.active ? (
                          <EyeSlashIcon className="h-4 w-4" />
                        ) : (
                          <EyeIcon className="h-4 w-4" />
                        )}
                      </button>
                      <button
                        onClick={() => handleDelete(manufacturer.id)}
                        disabled={deletingId === manufacturer.id}
                        className="text-red-600 hover:text-red-800 disabled:opacity-50"
                        title="Delete manufacturer"
                      >
                        <TrashIcon className="h-4 w-4" />
                      </button>
                    </div>
                  </div>
                </div>
              ))
            )}
          </div>

          {/* Desktop View */}
          <table className="hidden min-w-full text-gray-900 md:table">
            <thead className="rounded-lg text-left text-sm font-normal">
              <tr>
                <th scope="col" className="px-4 py-5 font-medium sm:pl-6">
                  Name
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  Code
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  Country
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  Email
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  Status
                </th>
                <th scope="col" className="px-3 py-5 font-medium">
                  Created
                </th>
                <th scope="col" className="relative py-3 pl-6 pr-3">
                  <span className="sr-only">Actions</span>
                </th>
              </tr>
            </thead>
            <tbody className="bg-white">
              {manufacturers.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                    No manufacturers found
                  </td>
                </tr>
              ) : (
                manufacturers.map((manufacturer) => (
                  <tr key={manufacturer.id} className="w-full border-b py-3 text-sm">
                    <td className="whitespace-nowrap py-3 pl-6 pr-3">
                      <div className="flex items-center gap-3">
                        <div>
                          <div className="font-medium text-gray-900">
                            {manufacturer.name}
                          </div>
                          {manufacturer.description && (
                            <div className="text-gray-500 text-xs">
                              {manufacturer.description}
                            </div>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-3 py-3">
                      <span className="font-mono text-sm bg-gray-100 px-2 py-1 rounded">
                        {manufacturer.code}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-gray-500">
                      {manufacturer.country}
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-gray-500">
                      {manufacturer.email || '-'}
                    </td>
                    <td className="whitespace-nowrap px-3 py-3">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        manufacturer.active 
                          ? 'bg-green-100 text-green-800' 
                          : 'bg-red-100 text-red-800'
                      }`}>
                        {manufacturer.active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-gray-500">
                      {formatDateToLocal(manufacturer.created_at)}
                    </td>
                    <td className="whitespace-nowrap py-3 pl-6 pr-3">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => onEdit(manufacturer)}
                          className="text-blue-600 hover:text-blue-800 p-1"
                          title="Edit manufacturer"
                        >
                          <PencilIcon className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => onToggleActive(manufacturer.id)}
                          className={`p-1 ${
                            manufacturer.active 
                              ? 'text-orange-600 hover:text-orange-800' 
                              : 'text-green-600 hover:text-green-800'
                          }`}
                          title={manufacturer.active ? 'Deactivate' : 'Activate'}
                        >
                          {manufacturer.active ? (
                            <EyeSlashIcon className="h-4 w-4" />
                          ) : (
                            <EyeIcon className="h-4 w-4" />
                          )}
                        </button>
                        <button
                          onClick={() => handleDelete(manufacturer.id)}
                          disabled={deletingId === manufacturer.id}
                          className="text-red-600 hover:text-red-800 disabled:opacity-50 p-1"
                          title="Delete manufacturer"
                        >
                          <TrashIcon className="h-4 w-4" />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
